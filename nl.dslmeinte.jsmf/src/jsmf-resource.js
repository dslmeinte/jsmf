...
jsmf.model.MResource = function(metaModel, localIdMap) {
	...
	this.notifyValueAdded = function(mList, index, value) {
		_.each(listeners, function(listener) {
			listener.notifyValueAdded(mList, index, value);
		});
	};

	this.notifyValueRemoved= function(mList, index, removedValue) {
		_.each(listeners, function(listener) {
			listener.notifyValueRemoved(mList, index, removedValue);
		});
	};

	this.notifyListChanged = function(mList, index) {
		_.each(listeners, function(listener) {
			listener.notifyListChanged(mList, index);
		});
	};

	this.notifyValueChanged = function(mObject, feature, oldValue, newValue) {
		_.each(listeners, function(listener) {
			listener.notifyValueChanged(mObject, feature, oldValue, newValue);
		});
	};
...
jsmf.model.Factory = function() {
	...
	function createMResource(modelJSON, metaModel, validationCallback) {
		...
		if( !_.isArray(modelJSON) ) throw 'model JSON is not an array of objects';
		_.each(modelJSON, function(item, index) {
			if( !_.isObject(item) ) throw 'non-Object encountered within model JSON array: index=' + index;
			_resource.contents.add(createMObject(item, null, null));
		});
		...
		function createMObject(initData, container, containingFeature) {	/* analogous to org.eclipse.emf.ecore.EObject (or org.eclipse.emf.ecore.impl.EObjectImpl / DynamicEObjectImpl) */
			...
			var validPropertyNames = _.map(_allFeatures, function(value, key) { return key; });
			...
			var setFeatureNames = {};
			_.map(_allFeatures, function(feature, featureName) {
			...
			_.map(initSettings, function(setting, settingName) {
			...
			function createNestedObject(feature, value, creator) {
				if( _.isArray(value) ) {
					...
					return new jsmf.model.MList(_resource, container, feature, _.map(value, function(nestedValue, index) {
